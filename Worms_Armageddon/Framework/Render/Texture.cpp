...
Texture::Texture(wstring file, Vector2 cutSizeXY, COLORREF color)
{
    HDC hdc = GetDC(hWnd);
    memDC = CreateCompatibleDC(hdc);
    alphaMemDC = CreateCompatibleDC(hdc);
    ...
    cutSize = { (long)cutSizeXY.x, (long)cutSizeXY.y };
    imageFrame = { imageSize.x / cutSize.x, imageSize.y / cutSize.y };
    transfColor = color;
...
Texture* Texture::Add(wstring file, Vector2 cutSize, COLORREF color)
{
    if (textures.count(file) > 0)
        return textures[file];

    Texture* texture = new Texture(file, cutSize, color);
...
float Texture::GetPixelHeight(const Vector2& pos, const int halfSizeY)
{
    int startY = pos.y < 0.0f ? 0 : (int)pos.y;
    int checkDepth = pos.y + halfSizeY;

    for (int y = startY; y < checkDepth; y++)
    {
        COLORREF color = GetPixel(memDC, (int)pos.x, y);
...
bool Texture::IsCollisionPixel(Vector2 pos)
{
    COLORREF color = GetPixel(memDC, pos.x, pos.y);
    if (color != transfColor)
        return true;
